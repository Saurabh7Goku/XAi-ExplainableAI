...
from app.core.xai import get_explainer as get_lime_explainer
...
async def predict_disease(
    ...
    try:
        ...
        # Generate LIME/Attention explanation
        explanation_result = get_lime_explainer().explain_image(file_path)
        explanation_image = explanation_result['image']
        explanation_mask = explanation_result['mask']
        
        # Save explanation image
        explanation_path = file_service.save_explanation_image(
            explanation_image, f"lime_{filename}"
        )
        ...
        # Convert explanation image to base64 for frontend
        lime_pil = Image.fromarray(explanation_image.astype('uint8'))
        buffer = io.BytesIO()
        ...
        return {
            ...
            "lime_mask": explanation_mask.tolist(),
